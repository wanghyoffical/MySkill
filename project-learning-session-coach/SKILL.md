...
4. 用户不知道学什么：按 `state/current.json.next_task` 给当天一个小任务；缺失时先用 `state/task_registry.json` 修复当前状态，再退回 `START_HERE.md`。
...
推进到下一任务时，必须用 `next_task_id` 到 `state/task_registry.json.tasks` 找完整任务对象。找不到就报告状态缺口，不要从 Markdown 临时拼一个任务。
