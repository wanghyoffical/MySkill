...
5. `state/task_registry.json`
6. `state/review_schedule.json`
7. `state/mastery.md`
8. `state/progress.md`
9. `ai/context.md`
10. `ai/coach-protocol.md`
11. `ai/concept-explanation-protocol.md`

如果文件缺失，直接指出缺哪个文件，并用现有文件继续，不要编造状态。

读取后先做状态一致性检查：
- `state/current.json.next_task_id` 必须存在。
- `next_task_id` 必须能在 `state/task_registry.json.tasks` 中找到。
- 如果 `state/current.json.next_task` 存在，它必须和 registry 展开的任务一致；不一致时先报告状态缺口并修状态，不继续学习。
...
4. 用户不知道学什么：用 `state/current.json.next_task_id` 到 `state/task_registry.json.tasks` 展开当天任务；缺失时先修复当前状态，再退回 `START_HERE.md`。
...
- `state/current.json`：更新当前任务 ID、下一任务 ID、registry 任务快照和阻塞点。
- `outputs/**`：只把候选模板补上证据槽，不伪造成果。

如果用户只是单点咨询，不推进学习状态，也要说明“本次未更新状态”。

推进到下一任务时，必须用当前 registry 任务的 `on_completion.next_task_id` 到 `state/task_registry.json.tasks` 找完整任务对象，并同步 `current_task_id`、`next_task_id`、`next_task`。找不到就报告状态缺口，不要从 Markdown 临时拼一个任务。
